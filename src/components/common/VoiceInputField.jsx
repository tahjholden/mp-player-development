...
const VoiceInputField = ({
  ...
  const getMicColor = () => {
    ...
    if (isRecording) {
      return recordingStatus === 'processing' ? 'success.main' : '#CFB53B'; // Old Gold color when recording
    }
  ...
  return (
    <Box sx={{ position: 'relative', ...sx }}>
      <TextField
        ...
        label={label}
        placeholder={`${placeholder}${placeholder ? ' ' : ''}(Tap the mic to dictate, or type manually)`}
        value={value}
        onChange={onChange}
        required={required}
        error={!!error}
      ...
      {showRecordingAnimation && (
        ...
        >
          ...
          >
            <CircularProgress size={24} sx={{ color: '#CFB53B' }} />
